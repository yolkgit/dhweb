...
import { COMPANY_INFO as INITIAL_INFO, PRODUCTS as INITIAL_PRODUCTS, FAQS as INITIAL_FAQS, DEFAULT_PLAYLISTS as INITIAL_PLAYLISTS, CERTIFICATIONS as INITIAL_CERTS, LAB_EQUIPMENT as INITIAL_LAB, HERO_SLIDES as INITIAL_SLIDES, LOGO_SETTINGS as INITIAL_LOGO, DESIGN_SETTINGS as INITIAL_DESIGN, CERT_MARKS as INITIAL_CERT_MARKS, INITIAL_CATEGORIES, CALCULATOR_SETTINGS as INITIAL_CALCULATOR, INITIAL_BRANCHES, DEFAULT_NAV_ITEMS, DEFAULT_GLOSSARY } from '../constants';
...
export const ContentProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [appSettings, setAppSettings] = useState<AppSettings>({ youtubeApiKey: '', navItems: DEFAULT_NAV_ITEMS, glossary: DEFAULT_GLOSSARY });
    ...
    const fetchData = async () => {
      ...
      if (data.appSettings && Object.keys(data.appSettings).length > 0) {
        ...
        setAppSettings({
          ...
          glossary: (incoming.glossary && incoming.glossary.length > 0) ? incoming.glossary : DEFAULT_GLOSSARY
        });
      }
